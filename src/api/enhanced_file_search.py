...
class EnhancedFileSearch:
    ...
    def __init__(self, files_directory: str = None):
        ...
        self.supported_formats = ['.json', '.md',
                                  '.txt', '.pdf', '.docx', '.doc']
        ...
        for filename in os.listdir(self.files_directory):
            ...
            try:
                ...
                if content:
                    ...
                    logger.debug(
                        f"Indexed file {filename} with metadata: {metadata}")
                else:
                    logger.warning(
                        f"Failed to extract content from file: {file_path}")
        ...
        for file_path, content in self.file_content_cache.items():
            ...
            if score > 0:
                ...
                for i, chunk in enumerate(chunks):
                    chunk_score = self._calculate_relevance(
                        chunk, query_terms, {})
