...
# Note: Evaluation models may not be available in all SDK versions
try:
...
class MyEventHandler(AsyncAgentEventHandler[str]):
    def __init__(self, ai_project: AIProjectClient, app_insights_conn_str: str, user_id: str, user_query: str):
        super().__init__()
    ...
    async def on_thread_message(self, message: ThreadMessage) -> Optional[str]:
        try:
            ...
            # Store the agent response for memory
            if message.role == "assistant":
                self.agent_response = stream_data.get('content', '')
    ...
    async def on_thread_run(self, run: ThreadRun) -> Optional[str]:
        ...
        if run.status == "completed":
            ...
            if self.agent_response and self.user_query:
                try:
                    ...
                    memory_manager.update_user_profile(
                        user_id=self.user_id,
                        # First 50 chars as topic
                        interaction_data={"topic": self.user_query[:50]}
                    )
                    logger.info(f"Stored memory for user {self.user_id}")
                except Exception as e:
...
async def chat(
    ...
    with tracer.start_as_current_span("chat_request"):
        ...
        # Get user query for memory management
        user_query = user_message.get('message', '')

        # Get relevant memories and format as context
        memory_context = memory_manager.format_context_for_agent(
            user_id, user_query)

        # Enhanced message content with memory context
        ...
        try:
            ...
            logger.info(
                f"Created message with memory context, message ID: {message.id}")
        except Exception as e:
        ...
        response = StreamingResponse(get_result(
            request, thread_id, agent_id, ai_project, app_insights_conn_str, carrier, user_id, user_query), headers=headers)

        # Update cookies to persist the thread and agent IDs and user ID.
        response.set_cookie("thread_id", thread_id)
        response.set_cookie("agent_id", agent_id)
...
async def get_user_profile(request: Request, user_id: str = Cookie(None), _=auth_dependency):
    """Get user profile with interaction history"""
    if not user_id:
        return JSONResponse(content={"profile": None, "message": "No user profile found"})
...
async def get_relevant_memories(
    ...
    try:
        memories = memory_manager.get_relevant_memories(user_id, query)
        return JSONResponse(content={"memories": memories})
    except Exception as e:
        logger.error(f"Error getting relevant memories: {e}")
        raise HTTPException(
            status_code=500, detail="Failed to get relevant memories")
...
async def get_memory_context(
    ...
    try:
        context = memory_manager.format_context_for_agent(user_id, query)
        return JSONResponse(content={"context": context})
    except Exception as e:
        logger.error(f"Error formatting memory context: {e}")
        raise HTTPException(
            status_code=500, detail="Failed to format memory context")
...
async def clear_user_memory(
    ...
    if not user_id:
        return JSONResponse(content={"status": "error", "message": "No user ID provided"})

    try:
        import os
        memory_path = memory_manager._get_user_memory_path(user_id)
        profile_path = os.path.join(
            memory_manager.storage_path, f"{user_id}_profile.json")

        # Remove memory and profile files
        ...
        raise HTTPException(
            status_code=500, detail="Failed to clear user memory")
