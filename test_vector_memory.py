...
def test_vector_memory_manager():
    ...
    try:
        ...
        memory_manager = VectorMemoryManager(
            storage_path=temp_dir, similarity_threshold=0.2)
        ...
        for i, test in enumerate(test_queries, 1):
            ...
            for j, memory in enumerate(relevant_memories, 1):
                ...
                query_preview = memory['query'][:50] + \
                    "..." if len(memory['query']) > 50 else memory['query']
                print(
                    f"      {j}. {query_preview} (similarity: {similarity:.3f})")
        ...
        print(
            f"   Memory stats: {profile['memory_stats']['total_memories']} memories")
        ...
        print(
            f"   Average query length: {analytics['avg_query_length']:.1f} characters")
        print(f"   Top topics: {analytics['top_topics'][:3]}")
        print(
            f"   Interaction frequency: {analytics['interaction_frequency']:.2f} per day")
        ...
        print(
            f"✅ Stored 20 additional memories in {performance_time:.3f} seconds")
        ...
        print(
            f"✅ Search on {len(memory_manager._load_user_memories('vector_test_user'))} memories: {search_time:.4f} seconds")
        ...
        print(f"\n📊 Performance Summary:")
        total_memories = len(
            memory_manager._load_user_memories("vector_test_user"))
        ...
        print(
            f"   Vector similarity threshold: {memory_manager.similarity_threshold}")
